...
import { ArrowRight, Palette, Building2, Briefcase } from 'lucide-react';
...
const roles = [
  { value: 'CREATOR' as const, label: 'Creator', icon: Palette, desc: 'Content creator or influencer' },
  { value: 'BRAND' as const, label: 'Brand', icon: Building2, desc: 'Business or product brand' },
  { value: 'AGENCY' as const, label: 'Agency', icon: Briefcase, desc: 'Talent or marketing agency' },
];
...
export function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left panel - branding */}
      <div className="hidden lg:flex lg:w-[45%] bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-500 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white w-full">
          ...
          <div className="space-y-6">
            <h2 className="text-3xl font-bold leading-tight">
              Join thousands of<br />creators & brands<br />growing together.
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-2xl bg-white/10 backdrop-blur p-5">
                <p className="text-3xl font-bold">10K+</p>
                <p className="text-sm text-white/70 mt-1">Active Creators</p>
              </div>
              <div className="rounded-2xl bg-white/10 backdrop-blur p-5">
                <p className="text-3xl font-bold">2K+</p>
                <p className="text-sm text-white/70 mt-1">Brand Partners</p>
              </div>
              <div className="rounded-2xl bg-white/10 backdrop-blur p-5">
                <p className="text-3xl font-bold">50K+</p>
                <p className="text-sm text-white/70 mt-1">Campaigns Run</p>
              </div>
              <div className="rounded-2xl bg-white/10 backdrop-blur p-5">
                <p className="text-3xl font-bold">95%</p>
                <p className="text-sm text-white/70 mt-1">Match Accuracy</p>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-slate-50 overflow-y-auto">
        <div className="w-full max-w-[460px]">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 mt-1">Get started free — no credit card required</p>
          </div>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center">
              <span className="bg-slate-50 px-4 text-xs text-slate-400 uppercase tracking-wider">or register with email</span>
            </div>
          </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {roles.map(({ value, label, icon: Icon, desc }) => (
                  ...
                  >
                    ...
                    <Icon className={`h-5 w-5 ${selectedRole === value ? 'text-indigo-600' : 'text-slate-400'}`} />
                    <span className={`text-sm font-semibold ${selectedRole === value ? 'text-indigo-700' : 'text-slate-700'}`}>{label}</span>
